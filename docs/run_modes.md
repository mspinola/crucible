...
![Cumulative R of the published rules: the full history rises through the era whose trends were planted for it, then the post-publication segment, judged on its own, drifts below zero.](img/stridsman_cumr.png){ width="660" }

![The gauntlet report for the post-publication window: a FAIL banner, the metrics row, and the REAL and STRONG gate blocks with their failing hard checks; DURABLE and GENERAL are labelled as not run rather than as failed.](img/stridsman_gates.png){ width="660" }
...
disclosed, is
[`examples/stridsman_postpub_yfinance.py`](https://github.com/mspinola/crucible/blob/main/examples/stridsman_postpub_yfinance.py)
