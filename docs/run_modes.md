...
A test pins the two facts that narrative depends on (a positive full-history expectancy
and a failed post-publication gauntlet) without pinning the formatting, so a change to
the simulator or the gates that flipped the story would fail CI rather than print a
false lesson. The numbers in the table are regenerated by running the example; they
