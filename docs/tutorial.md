...
| the optimizer's trial count | `n_variants`, which **you** declare, and nothing counts it for you (§5b) |
...
into thousands of alternate sequences, a spread of drawdowns and final equities. Notice what never
...
> The names collide, unhelpfully. §5a's technique *is* Monte Carlo, and Masters calls it the
...
log. (Sizing-dependent frictions, financing and market impact at size, belong to the capital
layer, §10.)
...
The same honesty tests apply whether your entries come from a rule or a model, and crucible only
