...
> Runnable versions live in [`examples/`](examples), indexed in its
> [README](examples/README.md): `quickstart.py`, `validation.py`, and `breadth.py` use
> synthetic data (no network). `real_data_yfinance.py` pulls real prices from Yahoo
> Finance (`pip install "crucible[examples]"`) and runs the full pipeline. Try
> `python examples/real_data_yfinance.py --ticker QQQ`.
